...
import { FaCalendarAlt, FaClock, FaMapMarkerAlt, FaPhoneAlt, FaUser, FaTractor, FaCheck, FaTimes, FaEye } from "react-icons/fa";
...
const AllBookings = () => {
  ...
  const [requests, setRequests] = useState([
    ...
    { 
      ...
      price: 9600,
      message: "Harvesting season is approaching. Need your harvester for my 5-acre wheat field.",
  ...
  const [bookingHistory, setBookingHistory] = useState([
    ...
    { 
      ...
      location: "Salem, Tamil Nadu",
      price: 9600,
      cancellationReason: "Farmer requested cancellation due to weather conditions",
      image: "https://5.imimg.com/data5/SELLER/Default/2021/1/KO/FV/IY/3042719/john-deere-w70-self-propelled-combine-harvester-500x500.jpg"
    },
  ]);
  ...
  const handleAction = (id, action) => {
    if (action === "accept") {
      ...
      if (rejectedBooking) {
        setBookingHistory([
          ...bookingHistory,
          { ...rejectedBooking, status: "Rejected", cancellationReason: "Owner declined the request" }
        ]);
        setRequests(requests.filter((req) => req.id !== id));
      }
    }
  ...
  const completeBooking = (id) => {
    const completedBooking = activeBookings.find((booking) => booking.id === id);
    if (completedBooking) {
      setBookingHistory([
        ...bookingHistory,
        ...
        <div className="bg-white rounded-lg shadow-md mb-6">
          <div className="flex border-b">
            {[
              ...
              { id: "history", label: "Booking History", count: bookingHistory.length }
            ].map((tab) => (
              <button
                key={tab.id}
              ...
              >
                ...
                {tab.count > 0 && (
                  <span className={`ml-2 px-2 py-0.5 text-xs rounded-full ${
                    tab.id === "requests" ? "bg-yellow-100 text-yellow-800" : "bg-gray-100 text-gray-800"
                  }`}>
                    {tab.count}
                  </span>
                )}
            ...
            {getActiveData().map((booking) => (
              ...
              >
                <div className="relative">
                  ...
                  />
                  <div className={`absolute top-0 right-0 ${getStatusColor(booking.status)} px-2 py-1 m-2 rounded text-xs font-medium`}>
                    {booking.status}
                  </div>
                </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="relative">
                ...
                <div className={`absolute top-2 left-2 ${getStatusColor(selectedBooking.status)} px-3 py-1 rounded-full text-xs font-medium`}>
                  {selectedBooking.status}
                </div>
              </div>
              
              <div className="p-6">
                <h2 className="text-2xl font-bold mb-4">{selectedBooking.vehicle}</h2>
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p>{new Date(selectedBooking.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                        </div>
                      </div>
                      <div className="flex items-center">
                        <FaClock className="mr-2 text-green-500" />
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <div className="flex items-center mb-2">
                        {[...Array(5)].map((_, i) => (
                          <svg
                            key={i}
                            className={`w-5 h-5 ${i < (selectedBooking.rating || 0) ? "text-yellow-400" : "text-gray-300"}`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
                          >
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      ...
                      <p className="text-blue-700">{selectedBooking.feedback}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="mb-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="flex justify-between mb-2">
                      ...
                      <span>₹{Math.round(selectedBooking.price * 0.05).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between font-bold pt-2 border-t">
                      <span>Total Amount</span>
                      <span>₹{Math.round(selectedBooking.price * 1.05).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-between">
                  ...
                  {selectedBooking.status === "Ongoing" && (
                    <>
                      <div>
                        <button
                          onClick={() => setSelectedBooking({...selectedBooking, showCancellation: true})}
                          className="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 transition-colors"
                        >
                          Cancel Booking
                        </button>
                ...
                {selectedBooking.showCancellation && (
                  <div className="mt-6 p-4 border border-red-200 rounded-lg bg-red-50">
                    <h3 className="font-semibold mb-2 text-red-700">Cancellation Reason</h3>
                    <textarea
                    ...
                    <div className="flex justify-end space-x-2">
                      <button
                        onClick={() => setSelectedBooking({...selectedBooking, showCancellation: false})}
